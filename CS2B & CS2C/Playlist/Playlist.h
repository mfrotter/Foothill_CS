...
public:
   ...
   class Song_Entry
   {
      ...
      public:
         ...
         bool set_id(int id) {if (id >= 0) _id = id; return id >= 0;}
         bool set_name(std::string name) {if (name != "") _name = name; return name !="";}
...
private:
   ...
   class Node
   {
      ...
      public:
         ...
         ~Node() {_next = nullptr;} // Do not do recursive free

         Song_Entry& get_song() { return _song; }
         Node *get_next() { return _next; }
         Node *insert_next(Node *p) {_next = p; return p;}
         Node *remove_next() {Node* successor = _next->get_next(); delete _next;  _next = successor; return this;}

         friend class Tests; // Don't remove this line
   };

private:
   Node *_head, *_tail, *_prev_to_current, *_t;
   size_t _size;
